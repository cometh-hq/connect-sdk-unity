...
namespace ComethSDK.Scripts.Services
{
	...
	{
		public static async Task<Signer> CreateSignerForUserId(string token, string userId, string walletAddress, API api,
			string provider)
		{
		...
		public static async Task<Signer> CreateSigner(string token, string userId, string walletAddress, API api,
			string provider)
		{
			if (string.IsNullOrEmpty(userId))
			{
